...
class MeshClient(object):
    def __init__(self, url, mailbox, password, shared_key=b"BackBone", ssl_context=None):
        self._url = url
        self._mailbox = mailbox
        self._ssl_context = ssl_context
        self._token_generator = _AuthTokenGenerator(
            shared_key, mailbox, password
        )

    def list_messages(self):
        req = Request("{}/messageexchange/{}/inbox".format(self._url, self._mailbox),
    ...
    def retrieve_message(self, message_id):
        ...
        req = Request("{}/messageexchange/{}/inbox/{}".format(self._url, self._mailbox, message_id),
                                     headers = {"Authorization": self._token_generator()},
                                     method="GET")
        return _Message(message_id, urlopen(req, context = self._ssl_context), self)

    def send_message(self, recipient, data):
        req = Request("{}/messageexchange/{}/outbox".format(self._url, self._mailbox),
                                     data,
                                     headers = {
    ...
    def acknowledge_message(self, message_id):
        message_id = getattr(message_id, "_msg_id", message_id)
        req = Request("{}/messageexchange/{}/inbox/{}/status/acknowledged".format(self._url, self._mailbox, message_id),
                                     headers = {
                                             "Authorization": self._token_generator(),
                                         },
                                     method="PUT")
...
class _Message(object):
    ...
    def __exit__(self, typ, value, traceback):
        try:
            ...
            self._response.__exit__(typ, value, traceback)
...
class _AuthTokenGenerator(object):
    ...
    def __call__(self):
        ...
        public_auth_data = _combine(
            self._mailbox, self._nonce, self._nonce_count, now
        )
        private_auth_data = _combine(
            self._mailbox, self._nonce, self._nonce_count, self._password, now
        )
        myhash = hmac.HMAC(
            self._key, private_auth_data.encode("ASCII"), sha256
        ).hexdigest()
        self._nonce_count += 1
